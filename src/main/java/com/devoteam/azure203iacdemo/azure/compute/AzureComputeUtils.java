...
public class AzureComputeUtils {

    private final Logger logger;

    public AzureComputeUtils(Logger logger) {
        this.logger = logger;
    }

    /**
     * Creates random resource name, based on prefix supplied.
     *
     * @param prefix to be used.
     * @return a max 30 character long random name.
     */
    public String createRandomName(final String prefix) {
        return SdkContext.randomResourceName(AzureMetaData.RESOURCE_GROUP_PREFIX + prefix, 30);
    }
    ...
    public void print(VirtualMachine resource) {
        ...
        logger.info(new StringBuilder().append("Virtual Machine: ").append(resource.id())
